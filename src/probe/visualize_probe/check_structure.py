...
def check_random_file():
    ...
    step_size = 0.064 * 3
    peaks = np.array([pp for pp, _ in peak_picking(torch.from_numpy(logits_boundaries_np), step_size) if logits_boundaries_np[pp] > 0.2])
    ...
    for i in range(num_segments):
        ...
        axs[0].fill_between(np.linspace(0, num_frames, num_frames),
                            (frames_true_np == i).astype(int),
                            color=segment_colors[i], label=segment_label)
    ...
    for i in range(logits_frames_np.shape[1]):
        ...
        axs[1].plot(np.linspace(0, num_frames, num_frames), logits_frames_np[:, i], color=segment_colors[i], label=segment_label)
    ...
    axs[2].plot(np.linspace(0, num_frames, num_frames), logits_boundaries_np , color='blue', label=number_to_label[0])
    ...
    axs[3].plot(np.linspace(0, num_frames, num_frames), logits_boundaries_np, color='orange', label='Logits Boundaries')
    axs[3].scatter(peaks, logits_boundaries_np[peaks], color='red', label='Peaks', zorder=5)
