...
class sLSTM(nn.Module):
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.zeros_(self.W_f.weight)
        bias_linspace_init_(self.W_f.bias, start=3.0, end=6.0)
        # input gate initialization
        torch.nn.init.zeros_(self.W_i.weight)
        torch.nn.init.normal_(self.W_i.bias, mean=0.0, std=0.1)
